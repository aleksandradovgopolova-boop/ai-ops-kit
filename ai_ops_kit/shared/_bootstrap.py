"""sys.path для модулей пакета: корень пакета (v4.0).
...
как в `validation/_bootstrap.py`.

Тёзки безопасны: оба ничего не хранят и лишь идемпотентно правят `sys.path`.

v4.0: плоский слой `tools/` СНЯТ (мажор). Он больше не подкладывается на путь: продуктовый код и
скрипты/документация child-репозитория перешли на пакетные точки входа (`python3 -m ai_ops_kit.<pkg>.<mod>`).
