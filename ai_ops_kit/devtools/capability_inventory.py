...
# НАМЕРЕННЫЙ карантин, а не «не хватило проводки». Некоторые built≠wired модули дормантны ПО РЕШЕНИЮ:
# их нельзя проводить в контур, пока не выполнено условие — и это объявленная граница, не долг. Без
# такой подписи ревью читает 0 импортёров как недоделку (так и случилось в аудите 17.09). Ключ —
# полное имя модуля; значение — краткая причина с указателем на решение/границу. Пусто для модуля =
# обычный built≠wired «ждёт проводки». Подпись — на витрине; сам факт дормантности держит
# tests/contracts/test_dormant_inventory.py.
INTENTIONAL_DORMANT_REASON: dict[str, str] = {
    f"{PKG_NAME}.security.security_review_cascade":
        "намеренный карантин (PARK #589): каскад НЕ прошёл held-out N=80 (пропуск уязвимости), "
        "строгую безопасность держит человек — объявленная граница портфеля `human_strict_security_5` "
        "(registry/release-claims.yaml). Проводить в контур ТОЛЬКО после нового held-out на более "
        "сильной модели с 0 unsafe_pass. Сохранён как воспроизводимый отрицательный эксперимент.",
}
...
# ── Главная мера продукта: карта из 5 метрик (kit-product-scorecard) ──────────────────────────────
# СДВИГ МЕРЫ (ревью 17.09, qualification/KIT-AS-PRODUCT-METRICS-MEASUREMENT-2026-09-17.md): заглавная
# мера продукта кита — не ЧИСЛО возможностей (`counts` выше), а карта из 5 продуктовых метрик
# (`intelligence/product_scorecard.build_scorecard`). Счёт возможностей ОСТАЁТСЯ виден как «ширина»
# (breadth) — сколько всего построено, — но это контекст, а не главный показатель. Самоотчёт
# (`self_report`) ВЕДЁТ картой, счёт идёт вспомогательно.
SCORECARD_HEADLINE = "kit-product-scorecard"

# Статическое ЗЕРКАЛО состава карты (id + человеческое имя + смысл) для committed-страницы. Живые
# ДОЛИ не впечатываем в страницу НАМЕРЕННО: они меняются с каждым планом/фичей/утверждением и краснили
# бы freshness-гейт витрины на не относящихся к делу PR. Живые значения даёт `./ai-ops scorecard` и
# `--json`. Синхронность этого зеркала с реальной картой держит контракт-тест (ids здесь == ids
# build_scorecard) — состав карты не разойдётся со страницей молча.
SCORECARD_METRICS: tuple[tuple[str, str, str], ...] = (
    ("feature_completion_rate", "Доля фич, дошедших от идеи до релиза",
     "движется ли продукт от намерения к выпуску"),
    ("evidence_coverage", "Доля утверждений кита с реальным evidence",
     "стоит ли за словами продукта доказательство"),
    ("product_memory_coverage", "Доля выпущенных фич с полной историей жизни",
     "помнит ли продукт, зачем и с каким итогом сделана фича"),
    ("outcome_coverage", "Доля фич с реальным результатом после релиза",
     "виден ли реальный исход выпуска — считается из отчётов о результате проекта"),
    ("learning_to_decision_rate", "Доля решений, опирающихся на прошлые результаты",
     "стал ли кит обучающимся, а не накопителем — считается из уроков, выведенных ИЗ результата"),
)


def _kit_claim_results():
    """Результаты проверки реестра честности (`knowledge/claims.yaml`) для доли покрытия evidence.

    Композиция ЖИВЁТ на этом слое (devtools=entrypoints) СОЗНАТЕЛЬНО, ровно как
    `ai_ops_cli_intents._kit_claim_results`: `validate_claims` — из `validation` (entrypoints), а
    `product_scorecard` (`intelligence`) её НЕ импортирует (это была бы зависимость вверх). Поэтому
    «прочитать реестр» делает entrypoint, а долю из него считает карта. Нет файла/сбой -> None
    (метрика 2 честно «не измерено», а не выдумка)."""
    from ai_ops_kit.validation import validate_claims
    claims_file = PKG / "knowledge" / "claims.yaml"
    if not claims_file.is_file():
        return None
    try:
        return validate_claims.build(claims_file)
    except Exception:  # noqa: BLE001 — один недоступный источник не роняет самоотчёт
        return None


def kit_scorecard() -> dict:
    """Живая карта продукта кита (5 метрик) — ГЛАВНАЯ мера самоотчёта. Read-only.

    Метрики 1/3 считаются из графа знаний, 2 — из реестра честности (читает этот слой, долю считает
    карта; см. `_kit_claim_results` о слоях), 4/5 — из отчётов о результате и уроков проекта."""
    from ai_ops_kit.intelligence import product_scorecard as ps
    return ps.build_scorecard(PKG, claim_results=_kit_claim_results())


def self_report() -> dict:
    """Самоотчёт кита о СЕБЕ как о продукте: ВЕДЁТ картой из 5 метрик (`headline`), а счёт
    возможностей идёт вспомогательно как «ширина» (`breadth`).

    Порядок ключей смысловой: заглавная мера первой, ширина — следом. Именно этот сдвиг просило
    ревью 17.09 — уйти от числа возможностей как главного показателя, не пряча его (ширина остаётся
    видимой). Read-only."""
    return {
        "schema_version": 1,
        "kind": "kit-self-report",
        "headline": {"measure": SCORECARD_HEADLINE, "scorecard": kit_scorecard()},
        "breadth": build_inventory(),
    }
...
**Главная мера продукта кита — не число возможностей, а карта из 5 продуктовых метрик**
(`kit-product-scorecard`, ниже и `./ai-ops scorecard`). Ревью 17.09 (`qualification/
KIT-AS-PRODUCT-METRICS-MEASUREMENT-2026-09-17.md`): кит перестаёт мерить развитие ЧИСЛОМ команд,
гейтов и валидаторов и начинает мерить себя как ПРОДУКТ. Поэтому эта витрина ведёт картой метрик, а
счёт возможностей ниже остаётся видимым как **ширина** — сколько всего построено, — но это КОНТЕКСТ,
а не заглавный показатель.

Витрина «что продукт умеет прямо сейчас» — достоверная: значения **выведены из реестров и кода** при
генерации, а не написаны руками, поэтому не расходятся с реальностью молча. Витрина read-only:
генератор только читает.
...
def render_markdown(inv: dict | None = None) -> str:
    ...
    # ГЛАВНАЯ мера идёт первой — карта из 5 метрик, а не счёт возможностей (ревью 17.09). Живые ДОЛИ
    # здесь НЕ впечатываем (они меняются с планом/фичами и краснили бы freshness-гейт витрины);
    # страница ведёт СОСТАВОМ карты и указывает, где взять живые значения. Счёт возможностей —
    # следом, как «ширина».
    lines = [_HEADER, "## Главная мера — карта продукта кита (5 метрик)", ""]
    lines += [
        "Кит мерит себя как ПРОДУКТ пятью метриками, а не числом возможностей. Живые доли (и честное "
        "«не измерено», где данных нет) — `./ai-ops scorecard` или "
        "`python3 -m ai_ops_kit.devtools.capability_inventory --json` (ключ `headline.scorecard`); "
        "метрики 1/3 считаются из графа знаний, 2 — из реестра честности, 4/5 — из отчётов о "
        "результате (features/<id>/outcome-readout.yaml) и выведенных из них уроков.",
        "",
        "| # | Метрика | Что показывает |",
        "|---|---|---|",
    ]
    for i, (_mid, title, meaning) in enumerate(SCORECARD_METRICS, 1):
        lines.append(f"| {i} | {title} | {meaning} |")
    lines += [
        "",
        "Ниже — **ширина возможностей**: сколько всего построено. Это контекст к карте (чем кит "
        "оперирует), а НЕ заглавный показатель развития.",
        "",
    ]

    lines += ["## Ширина возможностей (контекст, не главная мера)", ""]
    lines += [
        f"| Категория | Сколько |",
        f"|---|---|",
    ...
    if inv["built_not_wired"]:
        lines += [
            ...
            "Это НЕ приговор: обычно не хватает проводки — интента, гейта или записи в реестре. Но часть",
            "дормантна НАМЕРЕННО — по объявленному решению/границе (столбец «Причина»): такой модуль",
            "нельзя проводить в контур, пока не выполнено условие, и это не долг. Набор заморожен",
            "потолком-ратчетом; провели в контур — модуль обязан уйти из списка.",
            "Проверить: `tests/contracts/test_dormant_inventory.py` (`KNOWN_DORMANT`).",
            "",
            "| Модуль | Состояние | Причина |",
            "|---|---|---|",
        ]
        for m in inv["built_not_wired"]:
            rel = m.replace(".", "/") + ".py"
            reason = INTENTIONAL_DORMANT_REASON.get(
                m, "0 не-тестовых импортёров; ждёт проводки — "
                   "`tests/contracts/test_dormant_inventory.py`")
            lines.append(f"| `{rel}` | built≠wired | {reason} |")
        lines.append("")
    else:
...
def main(argv=None) -> int:
    ...
    if a.json:
        # JSON ВЕДЁТ картой продукта (`headline.scorecard`), счёт возможностей — под `breadth`. Это
        # тот же сдвиг меры, что и на странице: заглавный показатель — 5 метрик, не число.
        print(json.dumps(self_report(), ensure_ascii=False, indent=2, default=str))
        return 0
    inv = build_inventory()
