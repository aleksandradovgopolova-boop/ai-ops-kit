...
SCORECARD_METRICS: tuple[tuple[str, str, str], ...] = (
    ...
    ("outcome_coverage", "Доля фич с реальным результатом после релиза",
     "виден ли реальный исход выпуска (сейчас честный каркас «не измерено»)"),
    ("learning_to_decision_rate", "Доля решений, опирающихся на прошлые результаты",
     "стал ли кит обучающимся, а не накопителем (сейчас честный каркас «не измерено»)"),
)
...
def kit_scorecard() -> dict:
    ...
    карта; см. `_kit_claim_results` о слоях), 4/5 стоят честным каркасом «не измерено»."""
...
def render_markdown(inv: dict | None = None) -> str:
    ...
    lines += [
        ...
        "метрики 1/3 считаются из графа знаний, 2 — из реестра честности, 4/5 стоят честным каркасом "
        "«не измерено» (у самого кита живой аналитики ещё нет).",
        "",
