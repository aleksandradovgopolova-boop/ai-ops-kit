...
Гоняет N реальных задач через собранный движок (`ai_ops_run.py run … --engine pipeline --execute`
— флаг `--engine` принадлежит ВНУТРЕННЕЙ точке входа, у `ai-ops` его нет)
