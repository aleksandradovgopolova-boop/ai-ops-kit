...
# Группа переводчиков повседневных команд («что я собираюсь сделать», онбординг стека, каркас
# работы, план, описание задачи, discovery, ревью ветки, инженерный совет) вынесена в модуль-сосед
# `presenter_formatters.py`, чтобы presenter не рос как god-модуль. Здесь они реэкспортируются, и
# все вызовы `presenter.from_review(...)` / `getattr(presenter, "from_...")` продолжают работать.
#
...
_FORWARDED_FORMATTERS = (
    "from_advice", "from_discovery_draft", "from_execution_preview", "from_new_feature",
    "from_onboarding_profile", "from_plan_built", "from_review", "from_specification", "_CMD_RU",
)
