#!/usr/bin/env python3
"""Сверка issue-трекера с роадмапом: одна issue-эпик на направление + одна на каждую открытую работу.

ПОЧЕМУ КОМАНДА, А НЕ ПАМЯТЬ АГЕНТА. Правило владельца (10.09.2026): каждое открытое направление
роадмапа ВСЕГДА имеет issue, и это не должно держаться на том, что кто-то помнит завести. `roadmap
check` уже находит расхождение план↔ROADMAP.md; здесь — параллель для GitHub: расхождение
роадмап↔issues сводится одной командой.

ГРАНУЛЯРНОСТЬ (решение владельца): гибрид. Направление под «Сейчас»/«Следующий» = **эпик**; каждая
незакрытая работа (`todo`/`in_progress`) под ним = **подзадача**, связанная с эпиком task-list'ом.

АРХИТЕКТУРА — порты и адаптеры. Здесь ЧИСТАЯ оркестрация: `sync()` читает желаемое из отчёта
`roadmap_manager.check` и работ плана, читает существующее через инъектируемый `client` и возвращает
ПЛАН действий (создать/обновить/закрыть); мутации выполняются только при `apply=True`. Сеть (`gh`)
живёт в адаптере CLI, поэтому ядро тестируется на фейковом клиенте без GitHub — writer ≠ судья:
решает состав ядро, а выполняет сторона с побочными эффектами.

Идемпотентность: второй прогон подряд не создаёт и не меняет ничего (тело канонично и сверяется
точно). Ключ направления/работы вшит в тело HTML-комментарием `<!-- roadmap-sync: KEY -->`; для
issue, заведённых до команды вручную, ключ восстанавливается по заголовку/телу — команда их
УСЫНОВЛЯЕТ, а не плодит дубли.
"""
