...
class AnswersCorrupt(Exception):
    ...
    заполненный файл в живом прогоне niti (F-021).
    """
...
def read_answers(child_root) -> dict:
    ...
    try:
        ...
        # НЕРАЗОБРАННЫЙ ФАЙЛ — ЭТО НЕ «ОТВЕТОВ НЕТ» (F-021, живой прогон niti 2026-08-12).
...
def write_question_file(child_root, ask: dict):
    ...
    # Пусть ошибка дойдёт до человека словами «починить, а не отвечать заново» (F-021).
    ...
    for q in ask.get("questions") or []:
        ...
        # ЗНАЧЕНИЕ ПИШЕТСЯ ОДНОЙ СТРОКОЙ, БЕЗ МАРКЕРОВ ДОКУМЕНТА (F-021, живой прогон niti).
        #
