...
def gather_candidates(child_root) -> list:
    ...
    Все три проекции сами fail-safe (не бросают: `child_findings.project_findings` пропускает битые
    файлы канала, `roadmap_candidates.project_uncovered_directions` ловит PlanCorrupt и возвращает
    readable:false, `decision_candidates.project_decision_candidates` чист и не читает диск) —
    поэтому зовём их напрямую, БЕЗ широкого глушителя. Тихо гасить исключение значило бы «источник
    упал» выдать за «кандидатов нет» — ровно то, против чего кит. -> list[dict]."""
    ...
    out.extend(outcome_decision_candidates(root))
    return out
