...
def build_preview(intent, task, child_root, signals):
    ...
    # `run` неоднороден: роутер выбирает workflow по типу задачи, и не каждый заканчивается pull
    # request'ом. Для RESEARCH итог — не изменение кода, а доказательства и пакет для решения, поэтому
    # общий текст поставки («открою черновой pull request») здесь был бы утечкой чужого сценария.
    if intent == "run":
        expected = ("запущу исследование: превращу вопрос в доказательства и пакет для решения "
                    "(материалы соберу в .research/)"
                    if plan["base_workflow"] == "RESEARCH"
