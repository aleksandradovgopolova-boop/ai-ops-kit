...
def _kit_setup_line(verdict: dict) -> str:
    ...
    if verdict.get("verdict") == "valid":
        return ("отдельно — про настройку самого кита (обновление и фундамент): она в порядке; "
                "это не про продукт, детали покажу по запросу")
    return ("отдельно — про настройку самого кита (обновление и фундамент): часть ещё не закончена; "
            "это не про продукт, детали покажу по запросу")
...
def to_message(briefing: dict):
    ...
    technical = {"продуктовых рекомендаций": len(precs)}
