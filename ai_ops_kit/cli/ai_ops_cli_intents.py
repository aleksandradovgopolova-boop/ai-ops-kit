...
def _run_roadmap_sync(child_root, a):
    ...
    client = _GhIssueClient(child_root)
    try:
...
def _intent_roadmap(task, child_root, signals, a):
    js = a.json
    # Подкоманда sync-issues: сверка GitHub Issues с роадмапом (эпик на направление + подзадачи).
    if (task or "").strip().lower() == "sync-issues":
        return _run_roadmap_sync(child_root, a)
