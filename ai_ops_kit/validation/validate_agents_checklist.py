...
ALLOWED = (
    ...
    # Ночной показ задач-кандидатов — операционная джоба, а не проверка: она ПОКАЗЫВАЕТ владельцу
    # непокрытые направления роадмапа и открытые находки в сводке прогона по расписанию, у неё нет
    # вердикта pass/fail и она НИЧЕГО не пишет (только чтение; принимает кандидатов владелец через
    # `candidates accept`). Обернуть в pytest нельзя — есть показ, нет вердикта. Логика проекций
    # покрыта юнит-тестами (test_candidate_intake), ядро — чистые функции слоя intelligence.
    re.compile(r"^python3?\s+-m\s+ai_ops_kit\.cli\.ai_ops_cli\s+candidates\b"),
