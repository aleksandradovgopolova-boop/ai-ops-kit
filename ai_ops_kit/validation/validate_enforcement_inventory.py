...
Проверяется два инварианта против `registry/enforcement-inventory.yaml`:
  * ОХВАТ — каждый dev-инструмент (requirements-dev.txt + pyproject `dev = [...]`) и каждый
    ратчет-baseline (`packages/*baseline*.yaml`) перечислен в инвентаре; лишняя запись (объявления
    уже нет) тоже краснеет — реестр, разрешающий несуществующее, перестаёт что-либо значить;
  * РЕЗОЛЮЦИЯ — `enforced` инструмент обязан иметь `enforced_at`, чей паттерн реально находится в
    названном файле (доказательство «вызывается вот здесь» не может протухнуть молча); `unenforced`
    обязан иметь причину; каждый baseline обязан реально читаться своим `consumed_by`-валидатором.
...
def main(argv):
    ...
    print(f"ENFORCEMENT-INVENTORY-OK: {len(tools)} dev-инструментов "
          f"({unenf} честно unenforced), {len(spec.get('ratchet_baselines') or [])} baseline'ов — "
