...
Слияние идёт через родной GitHub merge queue (merge_group trigger в CI), а не через ручной
polling-цикл `gh pr merge`. Этот модуль только ПРОВЕРЯЕТ готовность PR к слиянию.
