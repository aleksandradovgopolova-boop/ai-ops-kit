...
def _build_containment(sandbox, pol, loop):
    ...
    return {"sandbox": sandbox, "shell_mode": pol.shell_mode,
            ...
            # R-43/#786: перечень ограничений тут обязан совпадать с честным списком в шапке
            # tool_broker.py — иначе кит обещает разное в двух местах. Добавлена невидимость
            # игнорируемых файлов: сторож их не откатывает (открытая часть R-43).
