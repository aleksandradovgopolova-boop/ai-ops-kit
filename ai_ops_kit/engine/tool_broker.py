...
# Откат может НЕ УДАТЬСЯ (права, каталог), и тогда отчёт это ГОВОРИТ: `fs_guard.revert_complete`
# false, а причина начинается с «ОТКАТ НЕ УДАЛСЯ» (R-43/#786 — раньше она безусловно заявляла
# успех). Успешный случай перечисляет откаченное: сторож видит не всё (см. пятый пункт ниже),
# поэтому список позволяет заметить, чего в нём нет.
#
...
#   * ИГНОРИРУЕМЫЕ файлы внутри protected-пути сторож НЕ ВИДИТ и потому не откатывает: снимок идёт
#     через `git status --porcelain -uall`, а тот игнорируемое не перечисляет (нужен --ignored).
#     Это R-43, и он ОТКРЫТ: здесь закрыта только отчётная половина (см. revert_complete выше),
#     покрытие — нет. Наивный `--ignored` не годится: сторож начал бы откатывать законную
#     игнорируемую суету (__pycache__, node_modules, артефакты сборки) на КАЖДОЙ операции.
        ...
        if op == "read":
            ...
            if _pre is not None:
                ...
                if _viol:
                    ...
                    ev["allowed"] = False
                    ev["ok"] = False
                    # R-43 (#786, решение владельца — вариант «починить отчёт»): причина
                    # говорила «правка откачена» БЕЗУСЛОВНО, в том числе когда откат провалился
                    # и правка ОСТАЛАСЬ на диске. Замер: команда создаёт файл в protected-пути и
                    # тем же вызовом делает каталог r-x -> unlink даёт EACCES, `failed` непустой,
                    # файл цел, а отчёт заявлял успех. Отчёт, называющий дыру закрытой, опаснее
                    # самой дыры: читающий «откачено» на диск не пойдёт (тот же класс, что R-33/
                    # R-38 — причина записана мягче факта).
                    # Второе: успешный случай теперь ПЕРЕЧИСЛЯЕТ откаченное. Сторож видит не всё
                    # (игнорируемые файлы ему невидимы — открытая часть R-43), поэтому «откачено:
                    # X» позволяет заметить, что Y в список не попал. Безличное «правка откачена»
                    # такой возможности не давало.
                    _names = "; ".join(f"{v['path']}: {v['reason']}" for v in _viol[:5])
                    _done = list(_undone["restored"]) + list(_undone["removed"])
                    _complete = not _undone["failed"]
                    ev["fs_guard"] = {"violations": _viol, "reverted": _undone,
                                      "revert_complete": _complete}
                    if not _complete:
                        ev["reason"] = (
                            "shell изменил запрещённые пути; ОТКАТ НЕ УДАЛСЯ — правка ОСТАЛАСЬ "
                            "на диске: " + "; ".join(str(x) for x in _undone["failed"][:5])
                            + " | нарушения: " + _names)
                    elif _done:
                        ev["reason"] = ("shell изменил запрещённые пути — откачено ("
                                        + ", ".join(_done[:5]) + "): " + _names)
                    else:
                        ev["reason"] = ("shell изменил запрещённые пути; откатывать было нечего "
                                        "(пути уже отсутствуют): " + _names)
