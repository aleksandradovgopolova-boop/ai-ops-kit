...
def make_reviewer_proposer(provider, gate_id, checklist="", required_evidence=None,
    ...
    def propose(context):
        prompt = (
            ...
            # УПРОЩЁННАЯ ЭЛИЦИТАЦИЯ (issue #614). Полевой замер 07.09: сложный структурный reviewer-result
            # с вложенными checks/evidence уводил живую модель в 56с многовитковой прозы БЕЗ машинного
            # вердикта — гейт не закрывался. Простой промпт «заверши одной строкой Recommendation: …»
            # дал чистый вердикт за 5с. Поэтому ПРОСТАЯ форма — основная и достаточная, а структурная —
            # опциональная (для заземления pass цитатой), но НЕ условие вердикта.
            "ПРОСТАЯ ФОРМА (основная, достаточная): заверши ответ РОВНО ОДНОЙ последней строкой-итогом\n"
            "  Recommendation: pass        — изменение можно принять\n"
            "  Recommendation: needs_work  — есть блокирующие замечания (назови их конкретно в прозе выше)\n"
            "Больше ничего после этой строки не пиши. Именно её кит берёт как вердикт.\n"
            "ОПЦИОНАЛЬНО можешь вместо строки завершить структурным reviewer-result — но ТОЛЬКО если "
            "тебе так удобнее; это НЕ обязательно и НЕ условие вердикта:\n"
            '  {"kind":"reviewer-result","gate":"' + gate_id + '","status":"pass|warn|fail",'
            '"checks":[{"id":"...","status":"pass|warn|fail","evidence":[{"file":"<изменённый файл>",'
            '"lines":"<диапазон>"}]}],"blockers":["..."]}\n'
            "Если хочешь провести файл (op:read) — верни РОВНО один JSON: "
            '{"op":"read","path":"..."}.\n'
            "Заземление pass: чтобы pass закрыл блокирующий гейт, вердикт должен опираться на "
            "прочитанный ДОСТАВЛЕННЫЙ файл — либо ты его прочитал, либо (в структурной форме) у check "
            "есть evidence-ссылка на КОНКРЕТНЫЙ файл из ЭТОГО изменения (file+lines): по ней кит сверяет, "
            "что вердикт коснулся доставленной правки, а не вынесен наугад. pass без опоры на изменение "
            "блокирующий гейт не закроет.\n"
