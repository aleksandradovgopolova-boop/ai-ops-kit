...
def make_reviewer_proposer(provider, gate_id, checklist="", required_evidence=None,
    ...
    def propose(context):
        prompt = (
            ...
            '"lines":"<диапазон>"}]}],"blockers":["..."]}  — ИТОГ\n'
            "Правила: читай минимально; выноси вердикт по фактам из прочитанного. У КАЖДОГО check "
            "ОБЯЗАТЕЛЬНА хотя бы одна evidence-ссылка на КОНКРЕТНЫЙ файл из ЭТОГО изменения (file+lines): "
            "по ней кит сверяет, что вердикт коснулся доставленной правки, а не вынесен наугад "
            "(ceremonial read-op у некоторых судей не фиксируется — заземление идёт по evidence). "
            "status=fail И "
            ...
            "изменение и КОНКРЕТНОЙ проблемы нет, это pass, а не warn «на всякий случай».\n"
            # ОБЯЗЫВАЮЩИЙ ФИНАЛ (полевой замер cockpit 8293fe54): гейт не закрывается НИ НА КАКОЙ
            # правке, если ревьюер не отдаёт РАЗБИРАЕМЫЙ вердикт. Поэтому итог обязан быть машинным и
            # ПОСЛЕДНИМ: если выше в ответе есть цитаты кода или примеры с фигурными скобками,
            # вердиктом считается ПОСЛЕДНИЙ reviewer-result, а не они.
            "ИТОГ обязателен и машиночитаем: заверши ответ РОВНО ОДНИМ reviewer-result — это твой "
            "вердикт. Если выше есть цитаты кода или примеры с фигурными скобками, вердиктом считается "
            "ПОСЛЕДНИЙ reviewer-result, а не они. Верни один JSON на шаг.\n\n"
