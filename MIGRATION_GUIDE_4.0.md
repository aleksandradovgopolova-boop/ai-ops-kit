...
4.0 — **мажор**, и обновление на него НЕ происходит само собой: guard `allowed_version_range` в
`.ai-ops.yaml` намеренно останавливает major-переход, чтобы breaking-изменение было твоим осознанным
решением, а не побочным эффектом ночного `update`. Есть два равнозначных пути — оба сознательные:

```bash
# из корня продуктового репозитория

# Путь A — расширить диапазон в .ai-ops.yaml (parent.allowed_version_range), например ">=3.0.0 <5.0.0",
# затем обычное обновление:
ai-ops update

# Путь B — разово форсировать major-переход, не трогая конфиг:
ai-ops update --force

ai-ops doctor          # подтверждает здоровье движка и managed-слоя на 4.0
```

`update` сам удаляет из дочки устаревший `.ai/managed/tools/` (он вышел из состава поставки) и
перегенерирует управляемые команды на пакетную форму.
