...
| 3 | Quote grounding | citation.quote в схеме (optional v0.2, конвенция для новых первичных EV, required с v0.3) + механическая проверка verify_quotes.py (re-fetch, substring/difflib); ретро-прогон: fetch_fail 0/41, пойман реальный дефект атрибуции (EV-312) | pytest: `test_research_tools_selftest.py` (verify_quotes) + структурная конвенция в validate_research_artifacts (warning); сетевой прогон — еженедельный watch; сетевой прогон — еженедельный watch |
| 4 | Freshness lifecycle | volatile→expires_at обязателен (CI, error); просроченный active — warning в CI, обработка — scheduled task research-freshness-sweep (Mon 9:00): stale/supersede/re-verify, лог в .research/watches/ | pytest: `test_research_tools_selftest.py` (freshness_sweep) + структурно в validate_research_artifacts; temporal — watch |
...
| 10 | Экономика конвейера | EV-ready разведка + ev_scaffold.py (19 EV за 2 команды в RR-007); роутинг по workflow_hint (quick/comparison/deep) | README роутинг-таблица; pytest: `test_research_tools_selftest.py` (ev_scaffold) |
...
запускаются набором (`tests/unit/test_research_tools_selftest.py`), а значит идут в каждой джобе,
