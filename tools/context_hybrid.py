...
def build_hybrid(v1_files, v2_view, allowed_classes, model_window=DEFAULT_MODEL_WINDOW):
    ...
    v1 = list(dict.fromkeys(v1_files or []))          # обязательный v1, уникально, порядок сохранён
    ...
    if not ready["ready"]:
        return {"mode": "v1-only", "promotion_ready": False,
                ...
                "mandatory_v1": v1, "v2_additions": [], "context": v1}
    ...
    return {"mode": "hybrid", "promotion_ready": True,
            "reason": "promotion gate ready -> mandatory v1 + разрешённые v2-additions",
            "contracts": ready["contracts"], "mandatory_v1": v1, "v2_additions": additions,
            "context": v1 + additions}


def build_hybrid_from_child(child_root, query, role, *, sha, afp, dcp=None, v1_mandatory=None,
                            budget=None, model_window=DEFAULT_MODEL_WINDOW, repo_id=None,
