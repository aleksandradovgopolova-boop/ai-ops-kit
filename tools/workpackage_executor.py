...
def execute_sequence(task, signals, child_root, packages, proposer_for, feature,
    ...
    доп. сигналы пакета (опц.). -> {kind, workitem_id, packages:[...], completed, stopped_at,
    executed_all, ready_all, final_sha, sequential_chain}."""
    import ai_ops_run
    ...
    for i, pkg in enumerate(ordered):
        ...
        sig_pkg["size"] = "small"
        # исполнитель = подтверждение декомпозиции: пакет атомарен, его id выбран (preflight пройдёт)
        ...
        # ОСТАНОВ последовательности только когда на пакете НЕЛЬЗЯ безопасно строить дальше:
        #   hard preflight-блок (модель не запускалась), нет коммита (пусто), ИЛИ регрессия (сломал базу).
        # «гейты требуют evidence» (нет author/review) — это НЕ стоп: пакет исполнен, но не ready.
        hard_blocked = rep.get("status") == "blocked"
        regressed = bool((rep.get("baseline") or {}).get("regressions"))
        executed = bool(sha) and not hard_blocked and not regressed
        ready = bool(rep.get("ready_for_pr"))
        blocked = hard_blocked or regressed
        ...
        status = ("preflight-blocked" if hard_blocked else
                  ("regressed" if regressed else ("executed" if executed else "no-commit")))
