...
**v3.34.0 stable** — Distribution Boundary Cleanup

- кит не занимает чужое имя: в колесе два top-level имени вместо трёх, `validation` уехал в пакет
- установка проверена по-настоящему: `pip install` в чистый venv, 170 модулей из 170
- замер циклов ядра стал ПОТОЛКОМ, а не заметкой: новая взаимная связь краснеет в CI
- доказательство релиза называет охват: `full-current-python` и `compatibility-matrix` — разное
...
- границы 14 пакетов имеют силу: направления зависимостей проверяются по реальному графу импортов
