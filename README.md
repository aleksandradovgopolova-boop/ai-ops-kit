...
**v3.33.0 stable** — Пакет стал пакетом

- `ai_ops_kit` импортируется как пакет — с одним корнем на пути, как после `pip install`
- границы 12 пакетов имеют силу: направления зависимостей проверяются по реальному графу импортов
- код движка живёт в `ai_ops_kit/` — 95 модулей, 12 пакетов; плоские имена остались алиасами
...
- 1507 pytest тестов (unit + contract + property-based)
