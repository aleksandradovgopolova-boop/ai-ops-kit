...
- **политика обновлений исполняется**: `update` при `update_policy: pr` больше не пишет в рабочее
  дерево владельца — обновление уходит в ветку `ai-ops/update-v<версия>` и ждёт PR (F-022)
- **опт-аут CI держится**: удалённый шаблон рекордера больше не возвращается на обновлении и не
  жжёт минуты Actions дочки (F-024)
- **нет ложного красного в чужом CI**: свой байткод больше не объявляется правкой владельца
  (F-025, issue #19) — причина была в двух реализациях одного вопроса
- **числа в документации проверяются во всех документах**, а не в одном (F-026); заготовка контекста
  больше не считается заполненностью, а свои `ProductStatus`/`now` заполнены замерами (F-027)
- **граница названа**: второй живой brownfield до verified PR не пройден — единственная незакрытая
  цель плана

Предыдущий релиз (v3.36.9, Own Medicine): кит перестал требовать от дочек того, чего не делал сам —
