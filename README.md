...
> **Честный статус движка (v2.117, 2026-07-18):** единый движок «задача → draft PR» собран,
> подключён к контроллеру и **проверяется ДЕТЕРМИНИРОВАННО в CI сквозными гарантиями продукта**
> (`validation/validate_product_qualification.py`, PQ1–PQ9). Ключевые свойства доказаны без живой
> модели:
...
>
> **Остаётся до тега v3.0-rc1 — на машине пользователя (не имитируем):** живые прогоны сценариев
> S1–S10 с моделью (DeepSeek через `openai-compatible`, `tools/qual_run.py`), настоящий draft PR
> (`--open-pr` + `GITHUB_TOKEN`), сохранённые JSON-отчёты. См. `docs/qualification-runbook.md`.
>
