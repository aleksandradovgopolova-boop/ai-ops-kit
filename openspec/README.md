...
выключенная по умолчанию** — текущий task-lifecycle продолжает работать без неё.
...
## Опция (выключено по умолчанию)
Включается в дочернем репо флагом `openspec.enabled: true` в `.ai-ops.yaml`
(профиль `core`, схема `spec-driven`). Пока флаг выключен — OpenSpec не используется,
работает прежний lifecycle.
