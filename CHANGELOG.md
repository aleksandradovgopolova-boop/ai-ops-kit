...
Пояс `PYTHONPATH` в workflow оставлен как есть: новый тест снимает окружение сам, поэтому закрасить
им этот класс дефектов больше нельзя. Тот же класс — «работает локально» из-за editable-установки —
разбирался в этом релизе уже дважды (`dcb08cb`, `run_report`); теперь он закрыт и со стороны
валидаторов.
