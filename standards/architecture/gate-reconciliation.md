...
| SEC-001 | Путь раскрытия уязвимостей в каждом репозитории | `required_repo_artifacts` | child (едет в дочку) | ai_ops_kit/planning/standard.py | да |
...
| SEC-008 | Архитектура безопасности документирована | `required_repo_artifacts` | child (едет в дочку) | ai_ops_kit/planning/standard.py | да |
...
_Итог: 38 статей — 12 с гейтом (6 доходят до дочки), 25 честный долг (#827)._
