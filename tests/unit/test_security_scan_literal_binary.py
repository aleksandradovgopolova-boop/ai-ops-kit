...
class TestNothingDangerousBecameSilent:
    ...
    @pytest.mark.parametrize("перенос,имя", [("\r\n", "виндовый"), ("\r", "старый макинтош"),
                                             ("\u2028", "юникодный разделитель строк")])
    def test_a_line_ending_does_not_move_the_address(self, перенос, имя):
        """ДЕВЯТЫЙ КРУГ РЕВЬЮ, И ЭТО РЕГРЕСС ЭТОЙ ЖЕ ВЕТКИ. Смещения строк считались как «длина без
        переноса плюс один», то есть закладывали ОДНОБАЙТОВЫЙ разделитель. На файле с виндовым
        переносом смещение недосчитывало байт на каждой строке, дрейф копился, и окно вызова
        читалось со сдвигом: разбор видел чужие байты и объявлял настоящую поверхность безопасной.

        Боевой адрес ИСЧЕЗАЛ из отчёта, а судья оставался на заведомо чистой строке выше — ровно тот
        механизм, ради закрытия которого работа и делается, только достигнутый байтовым сдвигом.
        На прежнем коде этого не было: там правило было построчным, без окон."""
        # Сканер зовётся НАПРЯМУЮ, без помощника `_флаги`: он дописывает свою строку импорта и
        # сдвинул бы номера — а здесь проверяется именно номер.
        строки = ['import { execFileSync, spawn } from "node:child_process";',
                  '  execFileSync("git", ["log"]);'] + [""] * 11 + ['  spawn(bin, argv);']
        флаги = security_scan.scan_injection({"srv.mjs": перенос.join(строки)})
        адреса = [f["line"] for f in флаги if f["id"] == "node_child_process_exec"]
        assert адреса == [14], f"{имя}: адрес уехал -> {адреса}"

    def test_the_url_signal_keeps_the_address_precise(self):
        """ПРИЗНАК `://` ПРОВЕРЯЕТСЯ АДРЕСОМ, А НЕ ФАКТОМ ФЛАГА. Без него `//` из адреса читается
        как комментарий, файл объявляется неразобранным, и флаг переезжает на строку ИМПОРТА —
        поверхность видна, но адрес хуже. Сторож, смотревший только «есть ли флаг», этого не
        замечал (назвало независимое ревью)."""
        код = ('execFileSync("git", ["log"]);\n'
               'const v = <p>см. http://x.ru</p>; spawn(bin, argv);\nconst z = 1;')
        правила = [id_ for id_, _ in _флаги(код, путь="server/ui.tsx")]
        assert "node_child_process_exec" in правила, правила
    ...
    def test_in_markup_a_comment_leaves_the_parse_unproven(self, код):
        """ПРИЗНАК РАЗМЕТКИ — ТОТ, ЧТО ЗАКРЫВАЕТ ВОСЬМУЮ И ДЕВЯТУЮ ДВЕРИ, и он не удерживался
        ничем: его снятие оставляло набор зелёным (назвало независимое ревью).

        В `.tsx` комментарий от текста не отличить, поэтому файл считается неразобранным и флаг с
        импорта не снимается. В обычном `.mjs` тот же код молчит — проверка рядом."""
        assert [id_ for id_, _ in _флаги(код, путь="server/ui.tsx")] == ["node_child_process"]
        assert _флаги(код, путь="srv.mjs") == []

    def test_the_addresses_of_both_readings_are_united(self):
        """ОБЪЕДИНЕНИЕ АДРЕСОВ ПРОВЕРЯЕТСЯ НАПРЯМУЮ. Через сканер это не поймать: прочтения на
        реальном тексте почти всегда дают один и тот же набор, и замена объединения на «последнее
        прочтение» проходила молча. Здесь два прочтения СПЕЦИАЛЬНО видят разные строки."""
        код = "spawn(a, b);\nspawn(c, d);\n"
        прочтение1 = ("spawn(a, b);\n            \n", True)
        прочтение2 = ("            \nspawn(c, d);\n", True)
        были, адреса = scan_exec_call.surface_lines(код, (прочтение1, прочтение2))
        assert адреса == [1, 2], адреса
        assert были is False, "прочтения разошлись — снимать флаг с импорта нельзя"

    def test_a_regexp_keeps_both_slashes_after_blanking(self):
        """ГРАНИЦЫ ГАШЕНИЯ ЛИТЕРАЛА ЗАКРЕПЛЕНЫ ТОЧНО. Гасится СОДЕРЖИМОЕ, косые остаются — по ним
        правило R-40 узнаёт `/…/.exec(`. Сдвиг границы на символ ломает это и ничем не краснел."""
        from ai_ops_kit.security.scan_prose import blank_comments
        assert blank_comments("const r = /abc/;", "a.mjs")[0] == "const r = /   /;"
        assert blank_comments("const r = /a/g.exec(s);", "a.mjs")[0] == "const r = / /g.exec(s);"

    def test_an_inline_regexp_exec_is_not_a_command(self):
        """R-40 на ВСТАВЛЕННОМ литерале, а не на именованном: прежний сторож проверял только
        `const RE = /x/; RE.exec(s)` и границы гашения не трогал."""
        assert _флаги('const m = /x/.exec(s);\nexecFileSync("git", a);') == []
