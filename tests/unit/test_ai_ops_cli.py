...
EXPECTED_INTENTS = {
    ...
    "contract",
    # 2026-09-11 (Claude-native онбординг для ОБНОВЛЁННОЙ дочки): `propose` — «предложение по
    # фундаменту» одним брифингом (что нового + ревью фундамента + вердикт + рекомендации +
    # Storybook). Оркестрирует готовые кирпичи (contract/next/ui_readiness) через presenter, read-only.
    "propose",
    ...
    "scorecard",
    # auto-slice-candidates: `candidates` — задачи-кандидаты из двух источников (непокрытые направления
    # роадмапа + наблюдения дочек) + приёмка пачкой. Без глагола `accept` план не меняется (кит сам не
