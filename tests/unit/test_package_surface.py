...
ZONE_NON_VALIDATORS = {
    ...
    "own_medicine_checks": "фундамент validate_own_medicine (константы+помощники+проверки по шагам "
                           "доставки); вынесен при разрезе монолита, импортируется фасадом ВНИЗ "
                           "(фасад→сателлит), сам точкой входа не является",
}
