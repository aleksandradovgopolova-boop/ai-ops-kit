...
def test_the_pr_title_is_gated_by_the_same_pattern_as_commits():
    ...
    ЗАМЕР 20.08.2026. PR #211 влит squash'ем; его человекочитаемый заголовок стал темой коммита на
    main. `cz check` в ветке смотрел коммиты ВЕТКИ — там всё было по формату — и о заголовке не
    знал ничего. После мержа main покраснел на lint, и КАЖДАЯ ветка от него унаследовала красный.
    Переписать защищённую историю нельзя, значит правило существовало и не исполнялось там, где
    решалось: то есть не работало.

    Проба стережёт ПАРУ. Точка включения сдвинута на один коммит только потому, что причина
    закрыта воротами на заголовок; сдвиг без ворот был бы дешёвым зелёным, а ворота без сдвига
    оставили бы main красным навсегда.
    """
...
@pytest.mark.unit
def test_moving_the_enforcement_point_carries_its_reason():
    """Точка включения формата — не число, которое двигают, когда мешает.

    Каждый её сдвиг обязан нести рядом записанную причину. Без этого правила гейт превращается в
    счётчик, который переставляют при каждом покраснении, и перестаёт значить что-либо.
    """
    txt = (KIT / "pyproject.toml").read_text(encoding="utf-8")
    i = txt.index("commit_format_enforced_after =")
    head = txt[max(0, i - 1400):i]
    assert "#" in head, "точка сдвинута без записанной причины"
    assert "squash" in head.lower() or "СДВИНУТО" in head, (
        "причина сдвига не названа рядом с самой точкой")
