...
def test_every_module_imports_with_root_only():
    """positive: пакет работает как пакет, а не как приложение к tools/."""
    modules = _modules()
    assert modules, "модулей не найдено — тест потерял предмет"
    failed = _import_with_root_only(modules)
    assert not failed, (
        "модули пакета не импортируются, когда на пути только корень "
