...
колеса нельзя было, пока код пакета звал валидаторов по плоскому имени. v3.34 это закрыл для
`validation`: каталог переехал под `ai_ops_kit/`, имён в колесе осталось два. Остался `tools` —
плоские алиасы точек входа; отдельный срез. Прежняя формулировка: перенос `validation/` под
...
KNOWN_TOPLEVEL = {"ai_ops_kit", "tools"}
