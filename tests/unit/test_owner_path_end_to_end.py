...
def test_owner_path(child):
    ...
    # ── шаг 6: валидатор из managed ДВАЖДЫ (F-025) ──────────────────────────────────────────────
    # Ключевой шаг, которого не было в clean-install: первый прогон создаёт `__pycache__` внутри
    # checksummed managed-слоя, второй обязан всё равно быть зелёным. Раньше второй краснел с
    # «managed-слой изменён вручную» — кит обвинял владельца своими артефактами.
    first = _validator_from_managed(child, "validate_ai_ops_child.py")
    assert first.returncode == 0, f"первый прогон валидатора красный:\n{first.stdout[-600:]}"
    assert list((child / ".ai" / "managed").rglob("__pycache__")), (
        "байткод не появился — предмет проверки пропал, шаг стал бессмысленным")
    second = _validator_from_managed(child, "validate_ai_ops_child.py")
    assert second.returncode == 0, (
        "ВТОРОЙ прогон валидатора красный — кит принял свой байткод за правку владельца:\n"
        + second.stdout[-600:])
