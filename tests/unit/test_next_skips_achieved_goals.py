...
def test_real_plan_is_not_advised_from_a_closed_goal():
    ...
    # ПУСТОЙ СОВЕТ ЗАКОНЕН, НО ОБЯЗАН БЫТЬ ОБЪЯСНЁН (замер 20.08.2026). Прежде здесь стояло
    # `assert best` — то есть тест требовал, чтобы на настоящем плане ВСЕГДА было что советовать.
    # Это не инвариант: план может быть законно исчерпан, и сегодня он такой — одна работа идёт у
    # владельца, вторая ждёт его решения по своему же условию.
    #
    # Инвариант этого файла — «не советовать из закрытой цели», и при пустом совете он выполняется
    # тривиально. Опасность тривиальной истины в том, что она молчит; поэтому пустой совет
