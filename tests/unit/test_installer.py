...
def test_delivery_footprint_is_smaller_than_legacy(installed):
    ...
    # 3.8 -> 3.85 МБ. Замер и решение владельца, не удобство.
    #   база C1+C2                         3 985 273 Б (3.8006 МБ)   уже у потолка 3.8
    ...
    # ПОЧЕМУ 3.85. База C1+C2 упёрлась в 3.8 вплотную (3.8006 в installed-замере — тот, что видит
    # этот тест, на ~63 КиБ больше managed_set из-за скиллов и шаблонов). Один валидатор дочки её
    # перебирает. 3.85 оставляет ~33 КиБ видимого запаса. Это ТРЕТИЙ подъём за сутки, и предыдущий
    # (3.7->3.8, коммит 664c3f0 #211) приехал БЕЗ строки в этой летописи — молча; здесь причина
    # записана, как требует правило файла.
