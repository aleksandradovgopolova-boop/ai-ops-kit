...
def test_delivery_footprint_is_smaller_than_legacy(installed):
    ...
    #
    # 2026-08-20, ЛЕНТА A: потолок ПОДНЯТ 3.7 -> 3.8 МБ. Записи рядом с числом не приехало — лента
    # замеров выше заканчивается 17.08 на 3.5 -> 3.7. Число здесь класса DERIVED, и правило этого
    # файла прямое: молча поднимать нельзя. Не переставляю чужое число (оно уже в main и работа под
    # ним смержена), но пробел называю: без записи следующий читатель не отличит замер от округления.
    #
    ...
    # 2026-08-20 (лента C, срез «форма вердикта»): ЗАПИСЬ ЗАМЕРА К УЖЕ ПОДНЯТОМУ ПОТОЛКУ.
    # Число 3.7 -> 3.8 приехало на main в 664c3f0 (#211) БЕЗ строки в этой летописи: потолок
    # двинулся молча, а правило этого файла — «поднимается только с содержательной причиной,
    # записанной рядом». Причину дописываю не задним числом за чужую работу, а за свою: ниже замер
    # среза, который в этот потолок упёрся, и основание, по которому он в дочку едет.
    #   origin/main + C1 (корпус gate-евалов)   3 871 634 Б (3.6923 МБ)   запас до 3.7 — 8 097 Б
    #   + C2 (контракт формы вердикта)          3 902 745 Б (3.7220 МБ)   +31 111 Б
    # ЧТО ИМЕННО ЕДЕТ В ДОЧКУ. Гейты дочки закрывает тот же судья, что и наши:
    # `providers/response_contract.py` (контракт формы + карта того, кто её умеет обеспечить +
    # типизированный отказ) исполняется В продуктовом репозитории на каждой judge-стадии. Оставить
    # его у кита значило бы повторить самый дорогой класс находок поля — механизм зелёный у нас,
    # отсутствующий у дочки (F-030/F-032). Данные корпуса gate-евалов, для сравнения, в поставку НЕ
    # поехали: они мерят судью кита на коммитах кита, дочке не нужны, и потолок не двигали (прирост
    # поставки от C1 — 4 600 Б).
    # 3.8 оставляет ~76 КиБ видимого запаса при трёх параллельных лентах в работе. Этот файл выше
    # трижды записал, чем кончается тонкий запас: 887 и 297 байт роняли тест на первой же следующей
    # работе, и «наказывать пришедшего последним значит мерить очередь, а не размер».
    # Смысл потолка сохранён: baseline монолита по ЧИСЛУ файлов (503) не тронут, потолок 475
    # содержательных файлов — тоже.
    # ГРАНИЦА НАЗВАНА: потолок по-прежнему ловит раздувание ПОСЛЕ пробоя и не умеет предупреждать
    # до него — работа `delivery-size-warns-before-breach` в плане не закрыта. Сегодняшний молчаливый
    # подъём в 664c3f0 — второй случай того же класса: сначала проехало незамеченным, потом
    # обнаружилось падением у следующего.
    # 2026-08-20 (лента C, срез C3 «что из мнения может стать машинным»): потолок ПОДНЯТ
    # 3.8 -> 3.85 МБ. Замер и решение владельца, не удобство. Числа СВЕРЕНЫ ПОСЛЕ слияния с main
    # (ленты A и B за день сдвинули базу — устаревшее число в этой летописи хуже отсутствующего).
    #   main перед этим срезом (после B)   3 940 013 Б (3.7573 МБ)   managed_set
    #   + documentation_evidence.py        +11 213 Б                 валидатор гейта, едет в дочку
    #   итог managed_set                   3 948 662 Б (3.7657 МБ)   запас до 3.85 — 88 КиБ
    # Тест видит installed (~63 КиБ больше managed_set: скиллы и шаблоны сверх managed_set) и на
    # объединённом дереве проходит под 3.85. Прежняя база C1+C2 (3.8006) заменена этой: она была
    # измерена до слияния лент A/B и дерево уже другое.
    # ЧТО ЕДЕТ И ПОЧЕМУ ЭТО НЕ «ПОДНЯТЬ, ЧТОБЫ ПРОШЛО». `documentation_updated` переведён из
    # самозаявления в машинный: его закрывает `ai_ops_kit/gates/documentation_evidence.py`, и эта
    # проверка исполняется В ДОЧКЕ на её дифе и её pyproject — то же основание, что у C2. Отчёт же
    # разбора (`devtools/gate_machinability.py`) и его данные (`quality/gate-machinability.yaml`) в
    # дочку НЕ едут: первый переведён в dev-only, вторые манифест и так не поставляет. Оставить
    # отчёт в `gates/` значило бы отправить дочке модуль без его данных — мёртвый груз (F-030/F-032).
    # ПОЧЕМУ 3.85. За сутки в этот потолок упёрлись все три ленты подряд: A (3.7->3.8, коммит
    # 664c3f0 #211 — БЕЗ строки в летописи, молча), B (`claim-reaches-other-copies`, уместилась под
    # 3.8) и C (этот срез, не уместилась). Это не «нужен запас», а замер: три доставляемых механизма
    # за день. 3.85 оставляет видимый запас, а не 887 байт, которыми этот тест уже дважды падал.
    # ГРАНИЦА НАЗВАНА: потолок ловит раздувание ПОСЛЕ пробоя и до него не предупреждает —
    # `delivery-size-warns-before-breach` в плане не закрыта, и три подъёма за сутки это подтвердили.
    assert total < 3.85 * 1024 * 1024, f"объём managed: {total / 1024 / 1024:.2f} МБ (потолок 3.85)"
