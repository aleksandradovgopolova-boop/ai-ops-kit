...
def test_delivery_footprint_is_smaller_than_legacy(installed):
    ...
    # 2026-08-20 (лента C, срез C3 «что из мнения может стать машинным»): потолок НЕ ПОДНЯТ.
    # Первая версия этой записи поднимала 3.8 -> 3.85 под валидатор `documentation_evidence.py`
    # (переводит гейт `documentation_updated` из самозаявления в машинный, едет в дочку). Пока
    # ветка шла, лента B в #218 отдала объём — сняла 60 КиБ балласта `patch_note` из
    # `release-claims.yaml`, — и на объединённом дереве C3 уместилась под 3.8 с запасом:
    #   managed_set после слияния B и C   3 888 388 Б (3.7083 МБ)   запас до 3.8 — 96 КиБ
    # Подъём отменён: потолок двигается только с содержательной причиной, а причины больше нет —
    # чужая уборка вместила мою работу. Оставить 3.85 «на всякий случай» значило бы сдвинуть число
    # класса DERIVED без основания, ровно против правила этого файла. Что документация о переводе
    # гейта — в `quality/gate-machinability.yaml` и newsfragment среза, а не в потолке.
