...
def test_delivery_footprint_is_smaller_than_legacy(installed):
    ...
    # v3.36.9: потолок ПОДНЯТ 3.2 -> 3.3 МБ осознанно и с замером. На `origin/main` поставка весила
    # 3 354 556 Б при потолке 3 355 443 Б — 887 БАЙТ ЗАПАСА, то есть потолок был исчерпан и любая
    # правка в `ai_ops_kit/` роняла его независимо от содержания. Повод для подъёма: измерение
    # расхода сессии по транскрипту рантайма (+26 КиБ в пяти модулях) — код, который ОБЯЗАН ехать в
    # дочку, потому что политику экономии сессии применяют в продуктовом репозитории, а не здесь.
    # Смысл потолка сохранён: baseline монолита ~3.6 МБ, и 3.22 МБ по-прежнему ощутимо меньше.
    # Молча поднимать нельзя — это число класса DERIVED, как и замеры слоёв.
    assert total < 3.3 * 1024 * 1024, f"объём managed: {total / 1024 / 1024:.2f} МБ"
