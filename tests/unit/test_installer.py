...
def test_delivery_footprint_is_smaller_than_legacy(installed):
    ...
    # 2026-08-20 (лента C, срез «форма вердикта»): потолок ПОДНЯТ 3.7 -> 3.8 МБ. Замер, а не
    # округление, и решение владельца, а не удобство ленты.
    ...
    # ЧТО ИМЕННО ЕДЕТ В ДОЧКУ И ПОЧЕМУ ЭТО НЕ «ПОДНЯТЬ, ЧТОБЫ ПРОШЛО». Гейты дочки закрывает тот же
    # судья, что и наши: `providers/response_contract.py` (контракт формы + карта того, кто её
    # умеет обеспечить + типизированный отказ) исполняется В продуктовом репозитории на каждой
    # judge-стадии. Оставить его у кита значило бы повторить самый дорогой класс находок поля —
    # механизм зелёный у нас, отсутствующий у дочки (F-030/F-032). Данные корпуса gate-евалов, для
    # сравнения, в поставку НЕ поехали: они мерят судью кита на коммитах кита, дочке не нужны, и
    # ради них потолок не двигался ни на байт (C1 дал прирост поставки 4 600 Б).
    # ПОЧЕМУ 3.8, А НЕ 3.75. 3.75 хватило бы ровно на C2 и оставило бы ~29 КиБ. Этот файл выше сам
    # трижды записал, чем кончается тонкий запас: 887 байт и 297 байт роняли тест на первой же
    # следующей работе, и «наказывать пришедшего последним значит мерить очередь, а не размер».
    # 3.8 оставляет ~81 КиБ видимого запаса при трёх параллельных лентах в работе.
    # Смысл потолка сохранён: baseline монолита по ЧИСЛУ файлов (503) не тронут, потолок 475
    # содержательных файлов выше — тоже.
    # ГРАНИЦА НАЗВАНА: этот потолок по-прежнему ловит раздувание ПОСЛЕ пробоя и не умеет
    # предупреждать до него — работа `delivery-size-warns-before-breach` в плане не закрыта.
