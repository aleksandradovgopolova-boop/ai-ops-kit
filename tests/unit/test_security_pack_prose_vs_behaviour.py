...
def test_behaviour_files_still_raise_domains_by_content():
    """границы: под-срабатывание НЕ введено — код с auth-логикой поднимает домен как раньше.
    ...
    ИЗМЕНЕНО 19.08.2026 (`security-gate-closable-on-quick`, замер на 240 настоящих коммитах): домен
    поднимается по-прежнему, но БЕЗ НАХОДОК и ТОЛЬКО по содержимому он теперь `advisory`, а не
    `needs_review`. Утверждение теста при этом не ослаблено, а уточнено: молчания как не было, так и
    нет — домен обязан быть НАЗВАН. Именно это здесь и проверяется, потому что «не блокирует» и
    «исчез» — разные вещи, и вторая вернула бы дефект v2.104.
    """
    r = run_pack(signals={}, files_content={"src/users.js": "const password = req.body.password"})
    assert "authentication" in r["advisory"], (
        f"auth-логика в коде перестала поднимать домен вовсе: {r}")
    assert r["overall"] == "advisory", (
        f"домен назван, но вердикт выдаёт его за проверенную чистоту: {r['overall']}")
