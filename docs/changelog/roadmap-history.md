# История roadmap — путь к AI Product Operating System (архив)

> **ФАЙЛ ЗАМОРОЖЕН 2026-08-11 на версии 3.36.7.** Это архив пройденного пути: 856 строк
> накопленных фаз с v1.3 по v3.36. Он больше не является объявлением направления и не
> поддерживается.
>
> **Направление продукта — в [`ROADMAP.md`](https://github.com/aleksandradovgopolova-boop/ai-ops-kit/blob/main/ROADMAP.md)** (четыре горизонта: Сейчас /
> Следующий результат / Дальше / Later). **Список релизов — в
> [`CHANGELOG.md`](https://github.com/aleksandradovgopolova-boop/ai-ops-kit/blob/main/CHANGELOG.md).**
>
> Любые слова «текущий», «ТЕКУЩАЯ ФАЗА», «остаётся», «намечен» ниже читать как состояние
> **на дату заморозки**, а не на сегодня. Текст оставлен дословно: он объясняет, ПОЧЕМУ
> решения принимались именно так, и эта причина не устаревает вместе с состоянием.
>
> **Почему файл разделён.** Документ одновременно объявлял направление и хранил историю, и это
> сломало обе роли: контракт `ai_ops_kit/planning/roadmap.py` не находил в нём ни одного из
> четырёх горизонтов (2 ошибки), а «ТЕКУЩАЯ ФАЗА» была помечена дважды и противоречиво —
> строка 750 говорила v3.6, строка 829 говорила v3.8, при выпущенной 3.36.7. Кит требует
> четыре горизонта от каждого подключённого репозитория; невыполнение собственного контракта —
> та же нечестная декларация, против которой стоят его инварианты. Разбор — в
> [`docs/audit-report.md`](../audit-report.md) (находка R-13).
...
совместимый в пределах 2.x; 3.x (текущий канал — **v3.36.7 stable**: Qualification Readiness + Trust Patch + Delivery Truth + Runner Agnostic + Work Is Seen + Delivery Proof Debt — правило, которое нельзя удовлетворить честно, не выключается, а признаётся долгом: отсутствие доказательства поставки объявляется отдельным состоянием, остаётся видимым и не блокирует то, что закрыть нечем; факт работы считается по git, а не по счётчику брокера (виден и свой коммит модели), поэтому «код не написан» больше не печатается при живом коммите; CI кита не предполагает, какая под ним машина (раннер выбирается переменной, python ставится только там, где это работает), а рецепты из документации проверяются на существование путей; исправление доезжает до уже подключённых репозиториев (`init` и `update` зовут одну доставку, и она стоит до раннего выхода «обновление не требуется»); shell больше не обходит protected_paths (последствия команды сверяются после выполнения, нарушение откатывается), child получает исполнимый CI, а публичная поверхность проверяется объявлением, а не совпадением подстроки; онбординг заканчивается работой, а не документацией (`ai-ops bootstrap` собирает первое направление и план из фактов аудита, продуктовые цели не выдумываются); репозиторий описывает себя сам (свои пути к плану и roadmap, свои источники истины, снятие нерелевантных сигналов кита); с человеком говорят все команды через один слой, а не четыре из двенадцати, и переводчик без вызова краснеет в тестах; находки гейта связности доходят до человека в выводе прогона. Предыдущие каналы: v3.35.0 Product Operating Model; v3.34.0 Distribution Boundary Cleanup — `validation` переехал в `ai_ops_kit/`, родовых имён в колесе два вместо трёх (`tools` остался объявленной границей); замер циклов ядра стал потолком (ратчет ходит только вниз), а опубликованные «119 циклов» оказались 31 циклом, посчитанным по разу с каждой вершины; доказательство релиза различает охваты full-current-python и compatibility-matrix; остаточный `.pth`-пояс кита в site-packages стал виден — `ai-ops doctor` проверяет гигиену путей первой строкой и блокирует, потому что пояс делает зелёными fail-closed-проверки; подчищать его автоматически решено НЕ будем (pip не может, installer не должен), лечение явное — `doctor --remove-path-belt`; инвариант трёх колец стал ПРОВЕРЯЕМЫМ (аналитика вынесена в пакет `intelligence` слоем выше ядра); пакет стал пакетом (и установка перестала прятать дефекты: setup.py писал .pth в site-packages пользователя на любом pip install) — внутренние импорты пакетные (257 штук), `ai_ops_kit` импортируется с одним корнем на пути; Layering — границы пакетов получили силу (слои foundation -> primitives -> capabilities -> entrypoints, проверка по реальному графу импортов; взаимные связи внутри ядра пока разрешены осознанно — замер 9 пар и 119 циклов); Package Layout ЗАВЕРШЁН — код движка в `ai_ops_kit/*` (95 модулей, 12 пакетов), плоские имена остались алиасами через sys.modules; вынос selftest из продакшн-модулей ЗАВЕРШЁН (26% -> 0, поставка 3.01 -> 2.52 МиБ); контур проверки сведён к одной команде `./scripts/check-full.sh` (чеклист 205 -> 0, группы CI 7 -> 4, быстрый цикл 4:40 -> 1:03); Proof of Fix ЗАВЕРШЁН — доказательство исправления (тест обязан падать на коде ДО правки, гейт regression_test_evidence advisory до обкатки), внешнее покрытие всех валидаторов (на 3.30 — 70 из 70); Field Findings ЗАВЕРШЁН — закрыты находки живых прогонов niti и ии-среды: честный резолв провайдера вместо хардкод-mock, полнота intake до старта прогона, база worktree от текущей ветки, lifecycle без ложного done, ложный traversal-deny, расхождение applied_writes с коммитом; публичные числа гейтов и покрытия валидаторов запиннены как DERIVED; Verification Foundation II ЗАВЕРШЁН — 230 pytest тестов (unit + contract + property-based), TypedDict контракты, coverage enabled, CI matrix (7 параллельных групп), orchestrator разбит на 4 модуля, pyproject.toml, kit observability, performance benchmarks, changelog automation; Verification Foundation ЗАВЕРШЁН — pytest + tests/ структура + 142 selftest wrappers + contract tests для critical path + CI layers (pr-smoke / full); Cost & Architecture Accuracy ЗАВЕРШЁН — UsageRecord расширен (task_type/workflow/risk/size/writer_tier/execution_mode/stack), parallel ledger fan-in, architecture baseline drift detection, architecture signals из diff; Engineering Advisor ЗАВЕРШЁН — `ai-ops advise` инженерный совет: окружения + delivery plan + альтернативы; Culture Runtime Integration ЗАВЕРШЁН — `ai-ops do` автономный прогон, session guard до старта, relation по факту, delegation advisor, opt-in session telemetry provider; Engineering Operating Model ЗАВЕРШЁН, срезы 1–3 (дисциплина коммита и веток с детектом отставания базы; карта окружений и честная зрелость поставки + гейт `deploy_readiness`; экономическая граница ДО траты в preflight; `ai-ops engops`); Development Culture & Resource Guardrails завершён (гигиена сессий/контекста: телеметрия, пороги бюджета, Task Completion Ritual + `ai-ops session`; boundary classifier + культура делегирования; cost-aware work method + `ai-ops method`); Architecture Baseline (v3.15.0); Startup Context Budget завершён v3.12–3.14; UI Evidence Readiness (v3.11.0); Usage Truth (честный учёт модельных расходов, v3.10.0); First-class Claude Code Adapter + complexity-aware routing (v3.9.0); фаза v3.8 Product Bootstrap завершена в 3.8.0; точная версия в VERSION) остаётся обратно совместимым —
...
| Writer ≠ judge, gates | 32 gate с revision-binding, machine-readable результаты (MVP-blocking = 8; счётчик запиннен claim gate-count/mvp-blocking-count) |
...
- **v3.22 → v3.31 — Verification & Layout** ✅ (перечисление по релизам — в `CHANGELOG.md`; здесь только
  итог, чтобы roadmap не заканчивался на 3.21): контур проверки перестал верить себе на слово —
  Verification Foundation I/II (pytest + contract + property-based тесты, TypedDict-контракты, coverage),
  ...
  Тестов 34 → 1519. Хотфикс 3.31.1: переезд сделал молчаливым no-op все 94 точки входа
  (алиас подменял собой `__main__`), поймано прогоном установки в чистом окружении — он и добавлен
  в контур отдельной джобой.
- **v3.36.0 — Qualification Readiness** ✅ (разбор перед реальной квалификацией, 2026-08-11):
  пять независимых ревью без авторского контекста и три обкатки на живых репозиториях дали ~30
  дефектов; ни одного не поймали 80 синтетических тестов. Четыре части.
  **(1) ГОВОРЯТ ВСЕ КОМАНДЫ.** Слой человеческого языка был подключён к трём командам из двенадцати;
  теперь путь наружу ровно один (`_say`), и держит это тест разводки, а не память автора команды.
  `from_doctor` был написан, покрыт тремя тестами и **не вызывался ниоткуда** — тесты держали код,
  который не работал; переводчик без вызова теперь краснеет. Вердикт `doctor` называет ПРИЧИНУ, а не
  считает строки с `✗`; шесть вердиктов ревью названы каждый своим именем — `no-ai-review-gates`
  значит «вливать можно, но не проверялось ничего».
  **(2) НАХОДКИ ГЕЙТА ДОШЛИ ДО ЧЕЛОВЕКА.** `contour_consistency` считал находки и писал evidence, а
  вывод прогона молчал: «описание отстало от кода» жило только в yaml. Гейт, чьи находки не видны, —
  это гейт, которого нет.
  **(3) РЕПОЗИТОРИЙ ОПИСЫВАЕТ СЕБЯ.** Пути плана и направления объявляются (монорепо прежде не мог
  описать себя вовсе); сигнал кита можно СНЯТЬ, а не только дополнить; опечатка в id контура — находка
  `major`, потому что ложная уверенность хуже пропуска; один обход дерева на вызов гейта вместо восьми.
  **(4) BOOTSTRAP СТАЛ КОМАНДОЙ.** Семь стадий первого сценария из девяти работали: кит не создавал ни
  `ROADMAP.md`, ни `planning/plan.yaml`. `ai-ops bootstrap` собирает их **из фактов аудита**; продуктовые
  цели не выдумываются — на их месте «нужно ваше слово». Интентов 14 → 15.
  Урок релиза: **локально зелёное не доказывает CI** — фикстура `features/A/` для работы с id `a` была
  зелёной на регистронезависимой macOS и красной на Linux четыре коммита подряд.

- **v3.35.0 — Product Operating Model** ✅ (аудит владельца 2026-08-10, четыре сообщения подряд):
  репозиторий описывает не только «что за продукт и как устроен», но и **куда развивается, что делать
  следующим и кто это делает**. Четыре части.
  **(1) МОДЕЛЬ.** `registry/product-operating-model.yaml` — восемь связанных контуров
  (Product & Strategy / Research & Decisions / Planning & Execution / System & Architecture /
  Data & Contracts / Engineering, Quality & Security / Delivery & Operations / Analytics & Learning),
  у каждого класс вопросов, источник истины, сигналы изменения, роль-владелец, реконструируемость и
  ярус достройки. Цикл ЗАМКНУТ: insight из аналитики меняет приоритет, приоритет меняет ROADMAP,
  ROADMAP меняет план — разрыв в любом месте превращает модель в документацию. Реестр, а не проза,
  потому что по нему работает код (детект, гейт, doctor, онбординг).
  **(2) PLANNING & EXECUTION СТАЛ АРТЕФАКТОМ.** Уровня между направлением и прогоном не было:
  WorkItem рождался из фразы пользователя, поэтому вопрос «что брать следующим» не имел входных
  данных. Теперь `ROADMAP.md` (четыре горизонта) + `planning/plan.yaml` (`kind: delivery-plan`) +
  `ai-ops next` — четыре вопроса в любой момент: где мы, что идёт сейчас, что блокирует и ЧЕМ, какую
  работу взять следующей и ПОЧЕМУ, что можно вести одновременно и какими ролями. Статус не
  объявляется там, где выводится: `ready`/`blocked`/`waiting` считаются из графа, WorkItem'а (гейты)
  и реестра активных работ; объявленный выводимый статус — видимое расхождение. План называет РОЛЬ —
  поля `runtime`/`model`/`provider`/`assignee` запрещены проверкой, иначе смена Claude Code на Codex
  переписывала бы план продукта. Параллельность — по тому же правилу, что ParallelSafetyDecision
  внутри задачи: непересекающиеся области записи + отсутствие зависимости.
  **(3) СВЯЗНОСТЬ КОНТУРОВ.** WorkItem объявляет `affects`; кит выводит затронутые контуры из diff и
  сверяет. Класс дефекта, который до этого не ловил никто: работа закрывается обновлением
  React-компонента при изменившейся модели данных. Три состояния, и `unknown` НЕ равен
  `not_changed` — контур без сигнальных путей кит видеть не умеет и говорит об этом
  (доопределяется в `.ai-ops.yaml`, а не выдумывается). Гейт `contour_consistency` — **advisory**
  (правило движения по roadmap), gate-count 33 -> 34.
  **(4) ПЕРВЫЙ СЦЕНАРИЙ + ЧЕЛОВЕЧЕСКИЙ ЯЗЫК.** «Установи AI Ops» разворачивается в
  INSTALL -> DISCOVER -> CLASSIFY -> RECONSTRUCT -> AUDIT -> ASK -> BOOTSTRAP -> PLAN -> RECOMMEND
  (`ai-ops model`): класс репозитория (NEW/EARLY/EXISTING/UNKNOWN, где UNKNOWN означает «не
  читается», а не «пустой»), реконструкция с PROVENANCE (`verified` — факт репо, `inferred` — вывод
  из структуры и повышению не подлежит, `unknown` — признание), аудит восьми контуров в семи
  состояниях, progressive Gap Plan вместо «заполните 14 документов», и ASK ONCE — один пакет
  вопросов только о том, что из кода не выводится, с предложением где есть основание. Закрытый
  контур вопросов НЕ задаёт. Наружу это выходит через Human Communication Layer
  (`registry/communication-policy.yaml` + `ai_ops_kit/ui/presenter.py`): контракт `UserMessage` в
  КОДЕ, а не только в скилле — иначе поведение теряется при смене runtime; аудитория по умолчанию
  `product`; простой язык НЕ означает сглаживание (`degraded` остаётся `degraded` на всех уровнях).
  Валидаторов 71 -> 72 (`validate_product_model`, развязан подпроцессом — ратчет слоёв остался 210),
  интентов 12 -> 15 (v3.35.2: `bootstrap`), тестов +80.
  **(5) РАЗБОР ПЕРЕД КВАЛИФИКАЦИЕЙ (v3.35.1–3.35.2).** Пять независимых ревью и три обкатки на живых
  репозиториях дали около тридцати дефектов, которых не поймали 80 синтетических тестов. Закрыто
  четырьмя тирами: швы данных и кириллица в путях; ложная уверенность тестов (покрытие мерило алиасы,
  а не `ai_ops_kit/`); человеческий язык во ВСЕХ командах, а не в четырёх; находки гейта связности
  дошли до человека; репозиторий получил право описать себя (`paths`, снятие сигналов кита, опечатки
  в id контуров, контракт сообщений из реестра, один обход дерева на вызов); и BOOTSTRAP, который был
  СТРОКОЙ, стал командой — `ai-ops bootstrap` создаёт направление и план из фактов аудита, не
  перезаписывая существующее и не выдумывая продуктовых целей.
  **Объявлено, НЕ сделано (следующий срез):** перевод `contour_consistency` в blocking по данным
  живых прогонов; вывод `affects` из спеки ДО старта работы (сейчас — из diff после); корпус
  eval-кейсов на пять аудиторий коммуникации; связь плана с knowledge graph; документы контекста
  bootstrap не пишет — только направление и план (документ без фактов был бы выдумкой).
