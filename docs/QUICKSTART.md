...
Для ENGINEERING/PRODUCT-задач добавьте `--review` (v2.83): движок отдельным read-only вызовом
модели выносит независимый вердикт по ai-review гейтам (code_review/ux_review/...) — writer ≠ judge.
Артефакт-гейты (requirements/spec/plan) и human-approval при этом честно остаются блокирующими,
пока нет соответствующих артефактов/человека — движок не выдаёт ложный «готово».
