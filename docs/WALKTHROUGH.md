...
сделанной, пока блокирующие проверки не доказаны**. Канонический вход — `ai-run "<задача>"`
(в дочке; движок делает всё остальное), а `/ai-start-task` в Claude Code — совместимый алиас;
ниже — та же механика через CLI, чтобы всё было воспроизводимо. `$KIT` — клон кита, `$PROJ` — тестовый проект.
...
По умолчанию провайдер стадий — **mock** (офлайн, детерминированный: для CI и разбора
механики). Живая модель — opt-in по ключу из окружения (секреты не в репо):
