...
его **не флипает**. Но молчать о пониженной изоляции нельзя: когда прогон реально работал (были
правки/коммит, не dry-run/preview) без песочницы, вывод прогона называет это прямо — одной честной
строкой рядом с `изоляция:`:

> ⚠ прогон без песочницы (sandbox off), сеть доступна модельному shell — для чувствительных
> прогонов включите изоляцию (--sandbox / контейнер run-sandboxed.sh)
