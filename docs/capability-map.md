...
## Главная мера — карта продукта кита (5 метрик)

Кит мерит себя как ПРОДУКТ пятью метриками, а не числом возможностей. Живые доли (и честное «не измерено», где данных нет) — `./ai-ops scorecard` или `python3 -m ai_ops_kit.devtools.capability_inventory --json` (ключ `headline.scorecard`); метрики 1/3 считаются из графа знаний, 2 — из реестра честности, 4/5 — из отчётов о результате (features/<id>/outcome-readout.yaml) и выведенных из них уроков.

| # | Метрика | Что показывает |
|---|---|---|
| 1 | Доля фич, дошедших от идеи до релиза | движется ли продукт от намерения к выпуску |
| 2 | Доля утверждений кита с реальным evidence | стоит ли за словами продукта доказательство |
| 3 | Доля выпущенных фич с полной историей жизни | помнит ли продукт, зачем и с каким итогом сделана фича |
| 4 | Доля фич с реальным результатом после релиза | виден ли реальный исход выпуска — считается из отчётов о результате проекта |
| 5 | Доля решений, опирающихся на прошлые результаты | стал ли кит обучающимся, а не накопителем — считается из уроков, выведенных ИЗ результата |

Ниже — **ширина возможностей**: сколько всего построено. Это контекст к карте (чем кит оперирует), а НЕ заглавный показатель развития.

## Ширина возможностей (контекст, не главная мера)

| Категория | Сколько |
|---|---|
| Команды владельца (`./ai-ops …`) | 37 |
...
|---|---|---|
| `ai_ops_kit/security/security_review_cascade.py` | built≠wired | намеренный карантин (PARK #589): каскад НЕ прошёл held-out N=80 (пропуск уязвимости), строгую безопасность держит человек — объявленная граница портфеля `human_strict_security_5` (registry/release-claims.yaml). Проводить в контур ТОЛЬКО после нового held-out на более сильной модели с 0 unsafe_pass. Сохранён как воспроизводимый отрицательный эксперимент. |
...
| `./ai-ops portfolio` | портфель отказов дочек обезличенно: какие классы повторяются по всем дочкам (только паттерны и числа, без кода и путей) | direct | built |
| `./ai-ops products` | флот продуктов: (без арг.) сводный вердикт по всем | register — добавить текущий репозиторий | direct | built |
| `./ai-ops propose` | что продукту нужно дальше: рекомендации человеческим языком (что важно, где ценность и риск) + можно ли двигаться; настройка кита и детали — по запросу | direct | built |
