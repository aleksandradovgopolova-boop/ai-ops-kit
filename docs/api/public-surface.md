...
2. **Интент `session` в дочке недостижим.** Его добавили в CLI (v3.36.13) именно ради установленной
   дочки, но обёртка дочки отправляет `session` в установщик, а установщик в поставку не едет. Без
   `AI_OPS_HOME` рядом дочка получает «исходник кита не найден» и код 2.

Оба относятся к `ai-ops` и `templates/runtime/ai-ops-entry.sh` — точкам входа, а не к границе.
