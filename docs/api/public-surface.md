...
experimental:
  ...
  cli_intents:
    ...
    - graph
    # kit-product-scorecard: `scorecard` — ЕДИНАЯ карта продукта кита из 5 метрик. Кит мерит СЕБЯ
    # как продукт (доля фич идея->релиз, доля утверждений с evidence, доля фич с полной историей
    # жизни), а не числом возможностей; метрики результата после релиза и «уроки->решения» честно
    # стоят «не измерено» (у самого кита живой аналитики нет). Только чтение; агрегат поверх графа
    # знаний и реестра честности. Форма карты ещё устаканивается — потому experimental, а не stable.
    - scorecard
