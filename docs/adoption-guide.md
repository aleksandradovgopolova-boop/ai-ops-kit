...
**Гейт отставания (с 18.08.2026).** `ai-ops check-update` — команда с кодом возврата, а не отчёт:
`0` — копия соответствует объявленной версии, `2` — отстала (разошлись версии, разошлось содержимое
при том же номере или managed-слой правили на месте), `1` — проверить не удалось («не знаю» тоже не
успех). Шаг уже стоит в `ai-ops-validate.yml`, который ставит `init`, и работает на клоне кита по
пину `installed_version`.

Зачем понадобился: второй по размеру класс находок поля — «исправление живёт в ките и не доезжает до
установленной копии», 8 находок из 48. Механизм обновления был, миграции были; не было команды,
которая ПАДАЕТ. Отчёт, который никого не останавливает, надеется на внимание человека.

Чего этот гейт НЕ делает, чтобы от него не ждали лишнего: он не краснеет от того, что у кита вышла
версия новее — это работа ежедневного `ai-ops-update.yml`, который открывает PR обновления. Иначе
каждый релиз кита краснил бы CI чужого продукта. Запускать гейт изнутри `.ai/managed/` бессмысленно
(копия сравнивалась бы сама с собой) — в этом случае он честно отвечает «не знаю» и называет, как
проверить правильно.
