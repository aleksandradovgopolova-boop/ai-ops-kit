...
через харнесс. Пример (S1, node/python-репо, режим `--sandbox` — это policy enforcement
брокера, не изоляция исполнения: сеть и ресурсы им не ограничены):
...
### 6e. Живой 3-пакетный sequential (S-SEQ): hard-stop + recovery — ПОДТВЕРЖДЕНО ЖИВЬЁМ (rc13)
...
> **Готча фикстуры (обязательно):** в корне репо нужен `conftest.py`, кладущий rootdir в `sys.path`
> (или `[tool.pytest.ini_options] pythonpath=["."]`). Без него движок гоняет pytest не из корня repo
> → пакеты не импортируются → baseline пакета 1 = сплошные collection-errors → чистый рефактор ложно
> выглядит регрессией (движок при этом честен: не false-green, а консервативный блок). `prep_seq_fixture.sh`
> это уже кладёт.

**Живые результаты (kimi-k3, 2026-07-21) — движок подтверждён по всем половинам:**

- **positive-green pkg-1:** `ready_for_pr=True`, валидные requirements+plan+specification → реальный
  рефактор (`DISCOUNT_RATES` вместо дублей) → **`code_review=pass`** → `unmet=[]`, `regressions=[]`.
  Т.е. полная строгая ENGINEERING-цепочка зелёная на живой сильной модели (на уровне пакета).
- **reviewer-block → hard-stop (rc13 P0):** на другом прогоне тот же pkg-1 получил `code_review=fail`
  с КОНКРЕТНЫМИ, точными блокерами (ревьюер поймал утечку `net_price` из scope pkg-2, недостающие
  regression-тесты, edge-case vip при amount<0) → `stop_reason=reviewer-blocked` → **pkg-2/pkg-3 не
  стартовали**. Именно тот rc11-случай (`warn`/`fail`-blocking), который до rc13 мог проскочить.
- **trusted retry → recovery (rc13 P1):** `--retry-package seqdemo-pkg-1-discounts` заархивировал
  заблокированную попытку (`work-packages/<pid>/attempts/attempt-1`), восстановил ветку на checkpoint
  предшественника **без ручного `git reset`**, перезапустил pkg-1 → чисто (`ready=True`) → цепочка
  продолжилась, `executed_all=True` по всем 3 пакетам.
- **provider crash contained (rc12):** ConnectionReset kimi в author-вызове → `stop_reason` типизирован
  (`network-error`, `retryable`), транзакция НЕ упала traceback'ом, снимки сохранены.

**Остаётся к `aggregate_ready=true` + настоящий draft PR:** одновременная зелень всех 3 пакетов на
одном прогоне — упирается в флак author-стадии kimi (rc14 добавил author-retry с нуджем, но при
сильной перегрузке нужен стабильный сильный провайдер). Это НЕ движковый блокер — каждый слой доказан.
...
- **Downstream секрет-сканер блокирует обновление кита** на словах `secret`/`token` или именах
  переменных (`GITHUB_TOKEN`, `OPENAI_API_KEY`) в документации — это слишком широкое keyword-правило, не
  утечка (в ките 0 реальных секретов, проверено). Исключите вендоренный managed-слой из скана или сузьте
  правило до value-based — см. `docs/downstream-secret-scanning.md`.
