...
Все цели помечены `freeze_relation: extension`.

> **Сверка с фактом при сведении с планом (20.08.2026).** Черновик исходил из того, что цели
> «встанут в очередь после снятия заморозки». Но заморозка `capability-freeze-until-second-brownfield`
> УЖЕ снята решением `ep-2026-08-19-freeze-lifted` — то есть `freeze_relation: extension` больше
> ничего не удерживает, и после сведения `next` сразу советует первую работу Phase 1
> (`artifact-registry-as-data`). Это не дефект сведения, а факт состояния: если владелец хочет,
> чтобы этот слой ждал закрытия текущей квалификации, нужна отдельная заморозка на эти цели —
> решение владельца, не молчаливое.
