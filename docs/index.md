...
- **Команды владельца.** Не помнишь команду — набери просто `ai-ops` (или `ai-ops help`): кит
  покажет короткую человеческую «дверь» — набор команд владельца простыми словами, а не стену из всех
  интентов. Полный список с флагами — `ai-ops help --all`. Каждый интент ещё и самоописывается
  (`./ai-ops <команда>`), а разбор потока — в [WALKTHROUGH.md](WALKTHROUGH.md).
  ...
  `do` / `run` (сделать задачу), `review` (проверить сделанное).
...
- [capability-map.md](capability-map.md) — одна достоверная витрина «что кит умеет сейчас»,
  ...
  planned и способ проверки. Ведёт **главной мерой продукта** — картой из 5 метрик
  (`./ai-ops scorecard`); счёт возможностей идёт следом как «ширина», не как заглавный показатель.
- [feature-catalog.md](feature-catalog.md) — человекочитаемый каталог фич продукта, собранный из
  реестра фич: по каждой фиче что делает / для кого / как проверить, поверхности и статус —
  чтобы аналитик разобрался во всех фичах из одного места (в ките — по образцу реестра, в дочке —
  по её собственному).
