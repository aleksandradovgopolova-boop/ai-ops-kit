...
- **Построено = проведено в контур (built≠wired).** Модуль `ai_ops_kit/**/*.py` считается
  ...
  входов-по-процессу: валидаторов `ai_ops_kit/validation/*`, dev-харнессов `ai_ops_kit/devtools/*`
  и поимённых CLI, которые зовёт `commands/**`. Прочий 0-импортерный код — дормантный инвентарь
  («судим больше, чем строим»): цель помечена достигнутой, а рабочего пути к коду нет. Текущий
  дормант заморожен потолком-ратчетом (`tests/contracts/test_dormant_inventory.py`): список вправе
  только сокращаться, новый built-not-wired краснеет. Отличается от достижимости
  (`test_capability_reachability`, где `__main__` уже делает модуль «достижимым») — здесь меряется
  ИМПОРТ, а не запускаемость.
...
в тот же день, и молчаливое удаление она встретит прогоном, а не чтением CHANGELOG. Пройденный
пример — плоский слой `tools/`: `deprecated` с 3.30, runtime-предупреждение в 3.40, физический снос
в мажоре 4.0 (`MIGRATION_GUIDE_4.0.md`; обновление через границу мажора — осознанное, `update --force`).
