#!/usr/bin/env bash
# Группа CI «selftests-m» — перенесённые селфтесты M–Z.
# v3.30: группы больше не перечисляют команды по одной. Всё, что раньше стояло здесь
# построчно, живёт в pytest; разбиение — по маркерам и именам, чтобы джобы шли параллельно
# и примерно равно по времени. Добавленная проверка попадает в свою группу сама.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1   # байткод в дереве ломает проверку целостности managed
cd "$(dirname "$0")/../.."
# ЗАМЕР ЦЕНЫ (2026-09-03): граница с группой a сдвинута l -> h (см. selftests-a.sh). Эта группа
# теперь берёт i..z (было m..z), чтобы выровнять время двух шардов. Партиция полная и непересекающаяся
# по построению: здесь ровно `not (то, что в группе a)`, так что ни один slow-тест не потерян и не
# гоняется дважды. Границу уточнить по фактическим временам следующего прогона.
python3 -m pytest -n auto --dist loadfile tests/ -q -m "slow and not nightly" -k "not (test_a or test_b or test_c or test_d or test_e or test_f or test_g or test_h or test_local_check or test_workpackage)"
