...
# Базой берём НОВЕЙШИЙ релизный тег репозитория. Прежде отсюда исключался тег текущей версии, но
# после намеренной чистки истории на stable (2026-09-08) единственный тег — v4.0.0, и он равен
# текущему номеру: рабочее дерево ушло вперёд на десятки коммитов при том же номере, поэтому
# обновление с v4.0.0 ПЕРЕ-ДОСТАВЛЯЕТ весь managed-слой (равная версия — не понижение). Пусто = ни
# одного релизного тега (а такого быть не должно — на stable остаётся минимум v4.0.0).
PREV_TAG="$(git -C "$KIT" tag --list 'v[0-9]*' --sort=-v:refname | head -1 || true)"
[ -n "$PREV_TAG" ] || fail "в репозитории нет ни одного релизного тега vX.Y.Z — не от чего обновляться (после чистки истории на stable должен остаться минимум v4.0.0)"
if [ "$PREV_TAG" = "v${CURRENT}" ]; then
  note "обновляем ${PREV_TAG} -> ${CURRENT} (рабочее дерево ушло вперёд при том же номере — пере-доставка managed-слоя)"
else
